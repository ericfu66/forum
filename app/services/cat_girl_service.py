...
class CatGirlService:
        ...
        if use_knowledge:
            try:
                ...
                if knowledge_context:
                    current_app.logger.info(f'Cat girl using knowledge context: {len(knowledge_context)} chars')
                    # 将知识库上下文添加到系统提示
                    system_prompt += f'\n\n===== 论坛相关知识 =====\n{knowledge_context}\n===== 知识结束 ====='
            except Exception as e:
                current_app.logger.error(f'Failed to get knowledge for cat girl: {e}')
    ...
    def generate_response_stream(self, user_message: str, conversation_history: list = None, 
                                   use_knowledge: bool = True) -> Generator[str, None, None]:
        """
        流式生成猫娘回复
        ...
        # 如果有知识库上下文，使用 chat_with_knowledge
        if knowledge_context:
    ...
    def auto_reply_async(self, user_id: int, user_message: str, message_id: int = None):
        ...
        """
        try:
            from app.tasks.cat_girl_tasks import auto_reply_task
            auto_reply_task(user_id, user_message, message_id)
        except Exception:
