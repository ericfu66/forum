...
def add_post_to_knowledge_async(post_id, title, content, board_name, author_name):
    ...
    """
    try:
        from app.tasks.knowledge_tasks import process_post_knowledge_task
        process_post_knowledge_task(post_id, title, content, board_name, author_name)
    except Exception:
        from flask import current_app
...
def search_similar_knowledge(
    ...
    if not query or not query.strip():
        current_app.logger.warning("Empty query provided for similarity search")
        return []
    
    # 先检查数据库中有多少条目
    total_post = PostKnowledge.query.count()
    total_custom = CustomKnowledge.query.filter_by(is_active=True).count()
    current_app.logger.info(f"[Knowledge] Database has {total_post} post entries, {total_custom} custom entries")
    
    # 向量化查询
    query_embedding = embed_text(query)
    if not query_embedding:
        current_app.logger.warning("[Knowledge] Failed to embed query, cannot perform vector search")
        return []
    
    current_app.logger.info(f"[Knowledge] Query embedded successfully, vector dim: {len(query_embedding)}")
    ...
    current_app.logger.info(f"[Knowledge] Found {len(post_entries)} post entries and {len(custom_entries)} custom entries WITH embeddings")
    
    if not post_entries and not custom_entries:
        current_app.logger.warning("[Knowledge] No entries with embeddings found - need to run vectorization!")
        return []
    ...
    for entry in post_entries:
        ...
        if vec:
            ...
            current_app.logger.warning(f"[Knowledge] Post entry {entry.post_id} has embedding field but failed to parse")
    ...
    for entry in custom_entries:
        ...
        if vec:
            ...
            current_app.logger.warning(f"[Knowledge] Custom entry {entry.id} has embedding field but failed to parse")
    
    current_app.logger.info(f"[Knowledge] Built {len(candidates)} valid candidates for similarity search")
    
    if not candidates:
        current_app.logger.warning("[Knowledge] No valid embedding vectors found in candidates")
        return []
    ...
    current_app.logger.info(f"[Knowledge] Similarity search returned {len(results)} results (threshold={threshold})")
...
def search_similar_posts(query_embedding: list, limit: int = 20, threshold: float = 0.3):
    """
    语义搜索帖子（返回Post对象和相似度分数）
    ...
    for entry, score in results:
        post = Post.query.get(entry.post_id)
