...
class AIChat {
    constructor(options = {}) {
        ...
        this.currentAssistant = options.defaultAssistant || 'forum';  // 默认使用论坛助手
        this.currentAssistantData = null;
        this.messageIdCounter = 0;
        this.useSearch = false;  // 联网搜索开关
        this.useSticker = false;  // 表情包开关
    ...
    init() {
        ...
        // 恢复用户之前选择的助手
        const savedAssistant = localStorage.getItem('ai-assistant');
        if (savedAssistant && this.options.assistants && this.options.assistants[savedAssistant]) {
            this.currentAssistant = savedAssistant;
        }
        // 同步 UI 状态
        this.syncAssistantUI();
    ...
    syncAssistantUI() {
        // 同步助手选择的 UI 状态
        const assistant = this.options.assistants ? this.options.assistants[this.currentAssistant] : null;
        if (assistant) {
            // 更新侧边栏助手列表的激活状态
            document.querySelectorAll('.assistant-item').forEach(el => el.classList.remove('active'));
            const activeEl = document.querySelector('[data-assistant="' + this.currentAssistant + '"]');
            if (activeEl) activeEl.classList.add('active');
            
            // 更新当前助手显示
            const iconEl = document.getElementById('current-assistant-icon');
            const nameEl = document.getElementById('current-assistant-name');
            if (iconEl) iconEl.textContent = assistant.icon;
            if (nameEl) nameEl.textContent = assistant.name;

            // 更新移动端显示
            const mobileIconEl = document.getElementById('mobile-assistant-icon');
            const mobileNameEl = document.getElementById('mobile-assistant-name');
    ...
    async streamResponse(userMessage, aiMessageId, images = []) {
        ...
        const requestBody = {
            ...
            use_sticker: this.useSticker  // 添加表情包开关
        };
        ...
        while (true) {
            ...
            for (const line of lines) {
                if (line.startsWith('data: ')) {
                    ...
                    // 检测表情包URL
                    const stickerMatch = data.match(/\[STICKER_URL:([^\]]+)\]/);
                    if (stickerMatch) {
                        stickerUrls.push(stickerMatch[1]);
                        continue;  // 不添加到内容中
                    }

                    const decodedData = data.replace(/\\n/g, '\n');
                    aiContent += decodedData;
                    this.updateMessageContent(aiMessageId, aiContent, stickerUrls);
                    this.scrollToBottom();
                }
            }
        }
    ...
    updateMessageContent(messageId, content, stickerUrls = []) {
        ...
        if (messageEl) {
            ...
            if (contentEl) {
                // 格式化内容，然后处理表情包标记
                let formattedHtml = this.formatContent(content);
                formattedHtml = processStickerMarkers(formattedHtml, stickerUrls);
                contentEl.innerHTML = formattedHtml;
                this.renderMath(messageEl);
                
                // 为代码块添加运行按钮
                if (typeof addRunButtonsToCodeBlocks === 'function') {
                    setTimeout(function() {
    ...
    formatContent(content) {
        ...
        // 处理连续的-开头项目（没有换行的情况，如：-项目1-项目2-项目3）
        // 先将连续的-项目转换为换行格式
        html = html.replace(/([^-\n])-([^\s-])/g, '$1\n- $2');
...
// 处理表情包标记，将[sticker:描述]替换为图片或占位符
function processStickerMarkers(html, stickerUrls) {
    if (!stickerUrls) stickerUrls = [];
    let stickerIndex = 0;
    
    return html.replace(/\[sticker:([^\]]+)\]/g, function(match, desc) {
        const url = stickerUrls[stickerIndex] || null;
        stickerIndex++;
        
        if (url) {
            return '<div class="sticker-container">' +
                '<img src="' + url + '" alt="' + desc.trim() + '" class="sticker-image" ' +
                'onclick="window.open(\'' + url + '\', \'_blank\')" title="' + desc.trim() + '">' +
                '</div>';
        } else {
            // 表情包正在生成中的占位符
            return '<div class="sticker-placeholder">' +
                '<span class="sticker-emoji">🎨</span>' +
                '<span class="sticker-desc">' + desc.trim() + '</span>' +
                '</div>';
        }
    });
}
